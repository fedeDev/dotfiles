...
DZEN="dzen2 -e 'button2=; -x $X -y $Y -w $WIDTH -h $HEIGHT -fn $FONT -ta right -bg $DZEN_BG -fg $DZEN_FG"
...
Vol ()
{
	...
	if [ -n "$MUTE" ] ; then
		echo -n "^fg($COLOR_ICON)^i($ICONPATH/vol1.xbm)^fg()" $(echo $VOL | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)
	else
		echo -n "^fg($COLOR_ICON)^i($ICONPATH/vol3.xbm)^fg()" $(echo $VOL | dzen2-gdbar -fg $BAR_MUTE -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)
	fi
	return
}
...
Disk ()
{
	...
	if [[ ${SDA2} -gt 80 ]] ; then
		echo -n "^fg($COLOR_ICON)^i($ICONPATH/file1.xbm)^fg() $(echo $SDA2 | dzen2-gdbar -fg $CRIT_COLOR -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)"
	else
		echo -n "^fg($COLOR_ICON)^i($ICONPATH/file1.xbm)^fg() $(echo $SDA2 | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)"
	fi
	#if [[ ${SDB1} -gt 80 ]] ; then
		#echo -n " ^fg($COLOR_ICON)^i($ICONPATH/home.xbm)^fg() $(echo $SDB1 | dzen2-gdbar -fg $CRIT_COLOR -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)"
	#else
		#echo -n " ^fg($COLOR_ICON)^i($ICONPATH/home.xbm)^fg() $(echo $SDB1 | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -nonl)"
	#fi
	return
}
...
Battery ()
{
	...
	if [ -z "$BATT_CONN" ] ; then
		echo -n "^fg($COLOR_ICON)^i($ICONPATH/ac5.xbm)^fg() $BATT_PERC%" # $(echo $BATT_PERC | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -ss 1 -nonl)"
	else
		if [[ $BATT_PERC -lt 85 && $BATT_PERC -gt 35 ]] ; then
			echo -n "^fg($COLOR_ICON)^i($ICONPATH/batt5half.xbm)^fg() $BATT_PERC%" #$(echo $BATT_PERC | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -ss 1 -nonl)"
		fi
		if [[ $BATT_PERC -lt 35 ]] ; then
			echo -n "^fg($COLOR_ICON)^i($ICONPATH/batt5empty.xbm)^fg() $BATT_PERC%" #$(echo $BATT_PERC | dzen2-gdbar -fg $BAR_FG -bg $BAR_BG -h $BAR_H -w $BAR_W -s o -sw 2 -ss 1 -nonl)"
		fi
